...
#In our setting, model = student model; ema = teacher model
class Trainer(object):
        ...
        with tqdm(initial=step, total=self.train_num_steps) as pbar:
            while step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    loss.backward()
                    #Let's see if gradient clipping will help?
    ...
    def sample(self, num, size_every, shape=None):
        ...
        for _ in range(num_cycle):
            #Change back to diffusion to test original ARMD
    ...
    def sample_forecast(self, raw_dataloader, shape=None):
        ...
        for idx, batch in enumerate(raw_dataloader):
            if len(batch)==2:
                ...
                x = batch
                x = x.to(self.device)
            sample = self.ema.ema_model.generate_mts(x) #Let's see what happens with the consistency model
            samples = np.row_stack([samples, sample.detach().cpu().numpy()])
            #reals = None
